// export default function Footer() {
//   return (
//     <div className="">
//       <div className="bg-sky-700 w-[1000px] h-[552px] rounded-xl flex justify-center"> </div>
//       <div></div>
//     </div>
//   );
// }

// export default function Footer() {
//   return (
//     <div className="relative min-h-screen flex flex-col items-center justify-center">
//       <div className="bg-blue-500 w-96 h-32 z-10">
//         <h1 className="text-white text-center pt-8">مستطیل وسط صفحه</h1>
//       </div>

//       <div className="bg-green-500 w-full h-[370px] absolute bottom-0 left-0 z-0">
//         <h1 className="text-white text-center pt-8">مستطیل پایین صفحه</h1>
//       </div>
//     </div>
//   );
// }



export default function Footer() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-between">
      <div className="bg-blue-500 w-96 h-32 flex items-center justify-center z-10">
        <h1 className="text-white text-center">مستطیل وسط صفحه</h1>
      </div>

      <div className="bg-green-500 w-full h-[370px] flex items-center justify-center z-0">
        <h1 className="text-white text-center">مستطیل پایین صفحه</h1>
      </div>
    </div>
  );
}
