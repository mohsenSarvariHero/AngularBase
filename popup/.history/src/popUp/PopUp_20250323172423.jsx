...
function PopUp({ onclose }) {
  ...
  const [showPassword, setShowPassword] = useState(true); // مقدار پیش‌فرض: false یعنی رمز مخفی است
  ...
  return (
    <div
      ref={modalref}
      onClick={closemodal}
      className="fixed inset-0 text-amber-50  backdrop-blur-xs flex justify-center items-center mb-5 "`fixed inset-0 text-amber-50 backdrop-blur-xs flex justify-center items-center mb-5 transition-all duration-300 ${
        isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'
    >
      <div className="mt-10 flex flex-col gap-5 text-black">
        <div className="rounded-xl bg-white flex flex-col gap-5 items-center mx-4 shadow-2xl w-[700px] h-[600px] mb-5">
          <h1
      ...
      <div className="relative">
        <input
          type={showPassword ? 'password' :  'text'} // در صورتی که showPassword برابر true باشد، رمز نمایان می‌شود
          id="password"
          required
        ...
        >
          {showPassword ? (
            <FaEyeSlash size={20} color="#3B82F6"  /> // نمایش آیکون چشم بسته
          ) : (
            <FaEye size={20} color="#3B82F6" /> // نمایش آیکون چشم باز
          )}
        </button>
      </div>
    </div>
