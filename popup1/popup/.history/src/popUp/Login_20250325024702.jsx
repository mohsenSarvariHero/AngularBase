...
function Login() {
  const [showPassword, setShowPassword] = useState(true);
  const [showModal, setShowModal] = useState(true); // برای کنترل نمایش یا عدم نمایش پاپ‌آپ
  const modalRef = useRef(); // مرجع به پاپ‌آپ

  const togglePasswordVisibility = () => {
    setShowPassword(!showPassword);
  };

  // این تابع بررسی می‌کند که آیا کلیک بیرون از پاپ‌آپ بوده یا نه
  const handleClickOutside = (e) => {
    if (modalRef.current && !modalRef.current.contains(e.target)) {
      setShowModal(false); // پاپ‌آپ بسته می‌شود
    }
  };

  useEffect(() => {
    // اضافه کردن event listener برای کلیک بیرون از پاپ‌آپ
  ...
  return (
    <div
      ...
      }`} // تنظیم opacity و مدیریت نمایش پاپ‌آپ
    >
      <div
        ref={modalRef} // ارجاع به پاپ‌آپ
        className={`mt-10 flex flex-col gap-5 text-black rounded-xl bg-white flex flex-col gap-5 items-center mx-4 shadow-2xl w-[700px] h-[600px] mb-5 transition-all duration-500 transform ${
          showModal ? "scale-100 opacity-100" : "scale-90 opacity-0"
        }`}
      >
        <h1
        ...
        <form className="flex flex-col">
          ...
          <div className="flex flex-col justify-center items-center mt-7">
            ...
            <Link
              className="text-[#3B82F6] text-[18px] font-extrabold mt-2 cursor-pointer"
              to="/SignupWithEmail"
            >
              ثبت نام
            </Link>
