...
function OTPCode({ onclose }) {
  ...
  useEffect(() => {
    ...
    const handleChange = (e, index) => {
      ...
      setValues(newValues); // به روزرسانی مقادیر ورودی

      // اگر عدد وارد شده و فیلد جاری پر باشد، به فیلد بعدی بروید
      if (value && index < 4) {
        document.getElementById(`input-${index + 1}`).focus(); // فوکوس به فیلد بعدی
      }
    };
      ...
      <div className="mt-10 flex flex-col gap-5 text-black">
        <div className="rounded-xl bg-white flex flex-col gap-5 items-center mx-4 shadow-2xl w-[700px] h-[600px] mb-5 transition-all duration-500 transform ">
            ...
            <div className="mt-5 -mb-3.5">
              {values.map((value, index) => (
                <input
                  ...
                  inputMode="numeric"
                  value={value}
                  onInput={(e) => handleChange(e, index)} // فراخوانی تابع هنگام وارد کردن داده
          ...
          <div className="w-[250px] flex flex-row items-end">
            <Link
              className="text-[12px] text-[#3B82F6] mr-5"
              to="/ResendOtpCode"
            >
              ارسال دوباره کد
            </Link>
            <p className="text-[#334155] text-[14px]"> 00: {seconds}</p>{" "}
            {/* تایمر شمارش معکوس */}
          </div>
