...
function ForgetPassword({ onclose }) {
  const modalref = useRef();
  const [showModal, setShowModal] = useState(false);
  const [inputValue, setInputValue] = useState(""); // وضعیت برای ذخیره ورودی کاربر
  ...
  // تابع برای بررسی ایمیل یا شماره موبایل
  const handleInputChange = (e) => {
    const value = e.target.value;
    setInputValue(value);

    // بررسی اینکه آیا ورودی یک ایمیل است یا شماره موبایل
    if (value.includes("@")) {
      setIsEmail(true); // اگر شامل @ بود، فرض می‌کنیم ایمیل است
    } else {
      setIsEmail(false); // در غیر این صورت، فرض می‌کنیم شماره موبایل است
    }
  };
          ...
          <form>
            <div className="relative flex flex-col mt-5">
              ...
              <input
                ...
                required
                value={inputValue}
                onChange={handleInputChange} // وقتی ورودی تغییر کرد
              />
            </div>

            {/* نمایش کامپوننت متفاوت بسته به ورودی */}
            {isEmail ? (
              <div className="mt-5 text-center">
                {/* اینجا صفحه یا کامپوننت ایمیل را نشان دهید */}
                <p>ایمیل وارد شده، فراموشی رمز عبور از طریق ایمیل ارسال خواهد شد.</p>
              </div>
            ) : (
              <div className="mt-5 text-center">
                {/* اینجا صفحه یا کامپوننت موبایل را نشان دهید */}
                <p>شماره موبایل وارد شده، فراموشی رمز عبور از طریق پیامک ارسال خواهد شد.</p>
              </div>
            )}
