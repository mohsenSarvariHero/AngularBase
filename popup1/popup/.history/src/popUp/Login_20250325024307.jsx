...
function Login({ onclose }) {
  ...
  const [showModal, setShowModal] = useState(true);  // به صورت پیش‌فرض پاپ‌آپ باز است

  // تابعی برای بسته شدن پاپ‌آپ وقتی روی صفحه کلیک می‌شود
  const handleClickOutside = (e) => {
    if (modalref.current && !modalref.current.contains(e.target)) {
      setShowModal(false);
      setTimeout(() => {
        onclose();  // تابع onclose برای اعمال تغییرات در والد
      }, 300);  // بعد از بسته شدن پاپ‌آپ، تابع onclose اجرا می‌شود
    }
  };
  ...
  useEffect(() => {
    // اضافه کردن listener برای کلیک‌های خارج از پاپ‌آپ
  ...
  return (
    <div
      className={`fixed inset-0 text-amber-50 backdrop-blur-xs flex justify-center items-center mb-5 transition-all duration-500 ${
        showModal ? "opacity-100 z-50" : "opacity-0 z-0"
      }`} 
    >
      <div
        ref={modalref}  // مرجع برای شناسایی پاپ‌آپ
