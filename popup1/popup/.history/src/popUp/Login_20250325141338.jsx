...
function Login({ onclose }) {  // onclose به عنوان prop
  ...
  const closemodal = (e) => {
    if (modalref.current === e.target) {
      setShowModal(false);
      setTimeout(() => {
        onclose(); // استفاده از onclose که از Enter کامپوننت دریافت شده
      }, 500);
    }
  };
  ...
  return (
    <div
      ref={modalref}
      onClick={closemodal}
      className={`fixed inset-0 text-amber-50 backdrop-blur-xs flex justify-center items-center mb-5 transition-all duration-500 ${showModal ? "opacity-100" : ""}`}
    >
