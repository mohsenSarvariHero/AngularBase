...
function App() {
  const [showPopup, setShowPopup] = useState(false);  // نمایش پاپ‌آپ
  const [currentPopup, setCurrentPopup] = useState(""); // برای تعیین پاپ‌آپ فعلی
    ...
    <Router>
      <div className="flex justify-center mt-10 text-amber-50">
        <Routes>
          ...
          <Route
            path="/Login"
            element={<Login onClose={closePopup} openPopup={openPopup} />}
          />
          <Route path="/ForgetPassword" element={<ForgetPassword />} />
          <Route path="/LoginWithOtpCode" element={<LoginWithOtpCode />} />
          <Route path="/OTPCode" element={<OTPCode />} />
        ...
        {showPopup && (
          <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
            ...
            >
              ...
              {currentPopup === "LoginWithOtpCode" && <LoginWithOtpCode />}
              {/* سایر پاپ‌آپ‌ها */}
            </div>
          </div>
        )}
